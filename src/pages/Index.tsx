...
import { Film, MessageSquare, Sparkles, Target } from "lucide-react";
...
const Index = () => {
  ...
  const getTabFromPath = () => {
    if (location.pathname === "/marketing-analysis") return "marketing-analysis";
    if (location.pathname === "/comment-analysis") return "comment-analysis";
  ...
  useEffect(() => {
    if (activeTab === "video-analysis") navigate("/video-analysis");
    if (activeTab === "marketing-analysis") navigate("/marketing-analysis");
    if (activeTab === "comment-analysis") navigate("/comment-analysis");
  }, [activeTab, navigate]);
  ...
  const getCanAnalyze = () => {
    switch (activeTab) {
      case "video-analysis":
        return (
          campaign && locality && selectedVideo && analysisOptions.length > 0
        );
      case "marketing-analysis":
        return (
          campaign && demographics.length > 0 && descriptors.length > 0 && selectedVideo
        );
      case "comment-analysis":
        return videoUrl && isValidYouTubeUrl(videoUrl);
      default:
        ...
        <div className="container mx-auto px-4">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="bg-transparent border-0 h-auto p-0">
              <TabsTrigger
                value="video-analysis"
                className="flex items-center space-x-2 px-6 py-3 rounded-none border-b-2 border-transparent data-[state=active]:border-primary"
              >
                <Film className="w-4 h-4" />
                <span className="hidden sm:inline">Video Analysis</span>
                <span className="sm:hidden">Video</span>
